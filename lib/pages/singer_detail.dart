  ...
  Widget contentAreaBuilder(context,snapshot){
    if(snapshot.hasData){
      singerInfo = snapshot.data['artist'];
          ...
          songList =snapshot.data['hotSongs'].map((item) => SongModel.fromJson(item)).toList();
          Provider.of<SongStore>(context).playList = songList;
      return Scaffold(
        body: CustomScrollView(
          ...
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              background: Image.network(artist.avatar,fit:BoxFit.cover,),
            )
          ,),
          SliverList(
            delegate: SliverChildBuilderDelegate(
              (BuildContext context,int index){
                return ListTile(
                  ...
                  onTap: (){
                    Provider.of<SongStore>(context).currentIndex = index;
                    Navigator.push(context, MaterialPageRoute(
                      builder: (context) => Player(songList:songList,index:index ),
                    ));
                  },
                );
              },
